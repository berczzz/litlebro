...
public class GrepFileTool implements AgentTool {
     ...
     * @return 带行号的匹配内容
     */
    @Tool(name = "grep_file", description = "在随消息上传的附件文件中按正则表达式检索内容，返回带行号的匹配行。适合在长文档中定位关键词后再用 read_file 读取上下文")
    public String grepFile(
            ...
            @ToolParam(description = "最多返回的匹配行数（超出截断），不传时使用默认值") int maxLines) {
